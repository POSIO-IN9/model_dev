...
for phase in phases:
    ...
    gt_path = os.path.join("wider_plant_split/wider_plant_{}.mat".format(phase))
    json_file = "{}.json".format(phase)

    gt = sio.loadmat(gt_path)
    event_list = gt.get("event_list")
    file_list = gt.get("file_list")
    plant_bbox_list = gt.get("plant_bbx_list")
    ...
    for event_idx, path in enumerate(event_list):
        ...
        for file_idx, img_name in enumerate(file_list[event_idx][0]):
            ...
            plant_bbox = plant_bbox_list[event_idx][0][file_idx][0]
            num_boxes = plant_bbox.shape[0]
            ...
            for i in range(num_boxes):
                xmin = int(plant_bbox[i][0])
                ymin = int(plant_bbox[i][1])
                xmax = int(plant_bbox[i][2]) + xmin
                ymax = int(plant_bbox[i][3]) + ymin
                w = xmax - xmin
                h = ymax - ymin
                area = w * h
                poly = [[xmin, ymin],
